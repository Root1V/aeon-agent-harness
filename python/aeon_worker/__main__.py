...
GraphRunWorkflow (RUN-002), DeepResearchWorkflow (DX-001), LangGraphInteropWorkflow (INT-001), and
their Activities. Also used directly (not via compose) by tests/integration/test_crash_resume.py
and the Temporal-integration tests under tests/integration/, which spawn this as a real OS
subprocess (the former to kill it and simulate a worker crash).
"""
...
from aeon_worker.activities.framework_adapter_activities import run_langgraph_interop_activity
...
async def main() -> None:
    ...
    worker = Worker(
        ...
        workflows=[AgentRunWorkflow, GraphRunWorkflow, DeepResearchWorkflow, LangGraphInteropWorkflow],
        activities=[
