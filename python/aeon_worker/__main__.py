...
CrewAIInteropWorkflow (INT-004), OpenAIAgentsInteropWorkflow (INT-005), and their Activities. Also
used directly (not via compose) by tests/integration/test_crash_resume.py and the
Temporal-integration tests under tests/integration/, which spawn this as a real OS subprocess (the
former to kill it and simulate a worker crash).
"""
