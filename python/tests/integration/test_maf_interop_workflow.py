"""INT-006's acceptance test: examples/maf-interop runs a real `agent_framework.Agent` loop inside
a Temporal Activity, against a real (ephemeral) Temporal server and a real separate worker OS
process — the only thing faked is the Model Gateway itself (a tiny local HTTP server standing in
for aeon-modelgw's real POST /v1/chat/completions, INT-002). Same fake wire shape as
test_openai_agents_interop_workflow.py (INT-005) — real OpenAI Chat Completions format, since MAF's
`OpenAIChatCompletionClient` speaks that format directly, not the internal /decide shape. The
Agent's own native tool-calling loop genuinely decides to call the Aeon tool (not an
Activity-driven fixed sequence); its output flows back through the real Activity/workflow boundary.
"""
...
class _FakeChatCompletionsHandler(BaseHTTPRequestHandler):
    ...
    tool-call response, and any later turn (after the SDK appends the assistant tool-call message
    and the tool's result) gets a final answer — same dispatch rule as
    test_openai_agents_interop_workflow.py's fake."""
...
@pytest.mark.asyncio
async def test_maf_interop_agent_runs_inside_a_real_activity():
    task_queue = f"aeon-maf-interop-test-{uuid.uuid4().hex[:8]}"
    ...
    try:
        ...
        async with await WorkflowEnvironment.start_local() as env:
            ...
            try:
                ...
                assert result.final_output, "the agent's own Agent.run never produced a final output"
