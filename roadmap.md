...
| OBS-009 | Un coste sin tarifar no puede contar como $0 en el circuit breaker | `TODO` | `TestUnpricedRunsDoNotSilenceTheCostBreaker` en verde. `A5` promedia coste por run y `recordOutcomeRequest.CostUSD` es `float64` con `omitempty`: un run sin tarifar llega como **0** y **baja la media**, así que cuanto más gasto no tarifado hay, más seguro parece todo. Es fail-open en un control de seguridad. **Medido el 2026-09-23**, con el mismo gasto real de $20 en los dos casos:
| caso | veredicto |
|---|---|
| 10 runs de $2.00 | `tripped=true` — «average cost $2.0000 over the last 5 run(s) exceeds threshold $1.0000» |
| los mismos $20 + 10 runs sin tarifar intercalados | `tripped=false` |
Y muerde justo donde vivimos: `prometheus_inference` es `compute_based`, así que **hoy toda llamada local es no tarifada** y contribuiría 0. **Latente, no activo**: `DefaultThresholds.MaxAvgCostUSD` es 0, que desactiva el chequeo de coste entero — el defecto se dispara el día que alguien lo active, que es cuando menos se mira. Mismo hallazgo que `OBS-008` una capa más arriba, y lo destapó Axonium contando que Prometheus tenía `sum(row["cost_usd"] or 0.0)` en su **página de facturación**: filas con guion, total `USD 0.00`. Su arreglo sirve de plantilla y cubre el caso no obvio: subtotal nulo cuando nada se tarifó, impuesto y cambio de moneda **propagan** el nulo en vez de multiplicar un desconocido, y una cobertura parcial viene acompañada de cuántas peticiones no cubre | — |
