...
| F4.5 | Costura Synaptum + Axonium (interop de runtime) | 31% (4/13) | `IN_PROGRESS` |
...
| MDL-011 | Verificación de modalidad del candidato antes de enrutar | `DONE` | `TestEmbeddingModelRoutedAsChatIsRejected` en verde — un candidato cuya modalidad no es de chat hace **fallar el perfil al resolverlo**, con `403` y tipo `aeon_policy_denied`, y el test comprueba lo que de verdad importa: **el proveedor no se llama ni una vez**. Hallazgo del equipo Axonium contra el despliegue real de Prometheus (`A-19`): `/v1/chat/completions` con un modelo de *embeddings* no falla, devuelve `200` con salida degenerada **que se factura** — y como la cascada de fallback solo avanza cuando un candidato *falla*, ese `200` degenerado nunca se reintenta: se acepta, se devuelve y el ledger de FinOps lo registra como llamada legítima. **`modality` es obligatorio**: el fallo que esto previene es precisamente un bundle que nunca dice qué es un modelo, así que el silencio se deniega igual que una modalidad equivocada. Un candidato malo **hace fallar el perfil entero**, no se salta: saltarlo sería ignorar en silencio lo que el operador escribió, el mismo error un nivel más arriba. **Límite honesto, escrito en el esquema:** verificamos la *declaración*, no la realidad — un modelo de embeddings declarado como `chat` se sigue enrutando. Contrastarlo con el catálogo vivo del proveedor es una feature aparte y más pesada | proto/schemas/model_profile.schema.json, go/internal/modelgateway/bundle.go, go/internal/api/openai_compatible_handlers.go |
...
| MDL-012 | Los tokens servidos desde caché existen en el ledger | `TODO` | `TestAnthropicCacheTokensReachTheLedger` en verde — hoy `anthropic.go` mapea `usage.input_tokens` a `prompt_tokens` y **ni siquiera parsea** `cache_read_input_tokens` / `cache_creation_input_tokens`, así que los tokens servidos desde caché **no existen** en el ledger de `OBS-003`. Y va en la peor dirección: [ADR-003](docs/adr/0003-context-lane-order-and-prompt-cache.md) diseña el Budgeter para explotar el prompt caching de Anthropic, así que el subregistro **crece cuanto mejor funcione la caché** — un error que nadie irá a buscar porque el gráfico mejora. Convención fijada con los otros dos equipos: `input` **incluye** lo cacheado y `cache_read` dice cuánto de ello lo fue. Ojo con la premisa: «el adaptador copia en vez de restar» vale para proveedores con forma OpenAI, pero **Anthropic reporta los contadores disjuntos** y ahí el adaptador tiene que **sumar** (a confirmar contra una respuesta real). Y el hallazgo de Axonium al escribir su propio test: distinguir «el backend no reporta caché» de «la caché estaba fría» — nulo no es cero, o se colapsan tres estados en dos. **No forma parte de `FND-004`**: este hueco existe hoy con la forma de dos contadores, independientemente de cuándo lleguen los cinco de `H3` | — |
| MDL-009 | Sustituir `prometheus_inference` nativo por Axonium-Go | `BLOCKED` | **`AXO-27` ya existe** — Axonium-Go v0.1, con la condición de aceptación que pusimos cumplida y medida (`upstream stopped after 3 chunks; the client read 3`), 60 tests, `-race` limpio y sin dependencias de terceros. Sigue bloqueado porque **el propio equipo de Axonium pide no desbloquearlo todavía**: falta cobertura al nivel de su implementación Python (78,7% frente a 100%), logging estructurado con spans OTel opcionales, y el tag `go/v0.1.0` publicado — `AXO-50`, `AXO-51` y `AXO-52`. El adaptador nativo actual —`DONE`, 392 líneas de producción + 244 de tests, verificado en vivo— se mantiene hasta que el reemplazo exista y pase el corpus de fixtures | — |
