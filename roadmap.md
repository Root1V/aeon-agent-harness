...
| F4.5 | Costura Synaptum + Axonium (interop de runtime) | 33% (5/15) | `IN_PROGRESS` |
...
| INT-012 | Corpus de normalización compartido ejecutado contra el gateway | `DONE` | `TestNormalizationCorpusAgainstThisAdapter` en verde — corre los 10 casos que publicó Synaptum (`contratos/normalizacion/`, leídos del fichero compartido, con los cuerpos que aportó Axonium) contra el adaptador `openai_compatible` real, a través de un servidor HTTP real para que se ejercite el camino de decodificación de verdad. Resultado honesto: **1 pasa, 8 con hueco, 1 divergencia conocida**. La distinción es el entregable — un **hueco** es un concepto que este adaptador no tiene (partes `thinking`, `tool_calls`, `estimated`, derivación desde `timings`) y es alcance de `FND-004`; una **divergencia** es un desacuerdo sobre algo que sí implementa, y de esas el corpus encontró tres: el motivo de fin ausente que no se mapeaba a `stop` (arreglado aquí, en los dos caminos), el `[DONE]` que salía sin emitir desenlace, y los ceros fabricados (`MDL-014`). El test **falla ante cualquier divergencia no registrada**; las conocidas se listan con nombre en vez de silenciarse, porque una divergencia acallada es indistinguible de una que nadie vio | go/internal/providers/openai_compatible/normalization_corpus_test.go, evals/contracts/normalizacion/ |
