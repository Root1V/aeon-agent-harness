...
| F4.6 | Del arnés al ejemplo real | 56% (5/9) | `IN_PROGRESS` |
...
| A2A-002 | Egress A2A gobernado: la delegación remota sale por el Tool Gateway | `TODO` | `TestRemoteDelegationIsGovernedAtEgress` en verde — una delegación a un agente remoto emitida por un framework que apunta al Tool Gateway se autoriza, se atribuye y se registra allí; una a un destino no permitido **no ocurre**, y la negación no depende de que el bucle llamante coopere. Abierto por la pregunta de Synaptum del 2026-09-20: hoy un agente gobernado no puede hacer nada no autorizado, pero **con delegación remota sí puede pedírselo a un tercero** — las herramientas del hijo cruzan *su* gateway, no el nuestro, y el efecto sale de nuestro perímetro en el `SendMessage`. **Forma decidida tras mirar la industria, y no es la que propuso Synaptum ni la que yo le contesté primero:** no un cliente A2A nuestro (opción 3, que le di el 19 y corregí el 20), ni una API de autorización que el framework consulta (opción 1, advisoria por construcción — un bucle con un fallo se la salta), sino **proxy en el camino de datos**: el framework apunta al endpoint y no cambia código, como hacen `agentgateway` y el «Agent-to-Anywhere» de Google Cloud Agent Gateway. Un control en el framework es una petición; un proxy en el camino es una frontera. Es **la tercera instancia de una forma que ya tenemos dos veces** (`INT-002` OpenAI-compatible, `INT-003` servidor MCP de salida), y por eso no necesita cliente A2A: el ciclo de vida de la tarea se queda en Synaptum. **No depende de que `A2A-001` se monte en un binario** — el proxy vive en el Tool Gateway, que sí está desplegado. Alcance mínimo que lo hace real: destino autorizado por Cedar, credencial inyectada por el gateway (nunca en el proceso gobernado), fila de coste por el mismo camino que el resto, e identidad por salto. Radio de explosión va aparte, y **el reparto lo señaló Synaptum el 2026-09-20**: tienen `max_delegation_depth` para el ciclo dentro de un proceso, y «un agente que delega en cincuenta a la vez no lo para nadie hoy». La profundidad la ve quien conoce el árbol (ellos); **el ancho lo ve quien está en el camino de todas las llamadas** (nosotros), porque cincuenta delegaciones simultáneas son cincuenta peticiones por el mismo proxy desde un proceso identificado. Ninguno de los dos lo ve entero por separado, así que el límite de fan-out es del arnés y no del framework. **Cuidado con el perímetro:** la delegación entre agentes suele ser este-oeste y no cruza el cortafuegos de salida, así que esto se sostiene por endpoint explícito y no por topología de red. **Restricciones de diseño acordadas con Synaptum el 2026-09-20, escritas antes de que exista el código porque son las que un proxy rompe al optimizar:** (a) un estado A2A desconocido se trata como `working` y **nunca como terminal** — con el mecanismo de extensión de la v1.0.1 aparecerán estados nuevos, y cerrarlos pierde el resultado en silencio; (b) `input_required` y `auth_required` **no son un resultado** y no se relevan como terminales — son una aprobación al otro lado de la red, y relevarlas haría que el padre continuara con una respuesta que el remoto nunca dio; (c) un agente remoto **se declara antes de que se le pueda delegar**, con riesgo explícito. En (c) divergimos de Synaptum a propósito: ellos aplican `DESTRUCTIVE` por defecto y nosotros exigimos la declaración en el registro, como ya hace `ToolDescriptor.risk` (obligatorio; una herramienta sin él no se registra). Las dos reglas son fail-safe, pero un defecto es silencioso —nadie se entera de que nunca se declaró— y un registro obliga a que una persona mire una vez, que es más valioso con un agente remoto que con una herramienta: al otro lado hay un modelo decidiendo y **puede cambiar sin avisarnos**. **Precondición confirmada:** el almacén es compartido por despliegue — un solo Postgres y el mismo `AEON_PG_DSN` en los cuatro servicios, y `run_checkpoints` tiene `PRIMARY KEY (run_id, step_id, phase)` **sin componente de contenedor**, así que una delegación remota interrumpida no se vuelve a pagar. Sin eso, esto no valdría más que recomendar un SDK | — |
...
| ID | Feature | Estado | Criterio de DONE | PR |
|---|---|---|---|---|
| MDL-017 | El núcleo de enrutado no conoce a ningún proveedor por su nombre | `DONE` | `TestUndeclaredLocalInferenceProviderIsDenied` y `TestModelGatewayRoutingFallback` en verde con nombres de proveedor **arbitrarios** — antes pasaban por la razón equivocada. `gateway.go` tenía `const restrictedProvider = "prometheus_inference"`: el núcleo de enrutado con el nombre de una plataforma compilado dentro, y `MDL-008` construido encima dándole a ese proveedor el privilegio de servir inferencia local sin declararlo. Cumplía la **letra** de [ADR-004](docs/adr/0004-model-gateway-provider-abstraction.md) —este paquete no importa ningún adaptador— y rompía su **razón de ser**: un arnés que se sabe de memoria el nombre de una plataforma está acoplado a ella, la importe o no, y sirve a una sola organización mientras dice ser agnóstico. Ahora las dos reglas se declaran en el `ModelPolicyBundle` (`routing_constraints.in_network_providers` y `local_inference.allowed_providers`) y el núcleo solo las aplica. **Ningún proveedor tiene privilegio por nacimiento, tampoco el de la casa.** Se conservan las dos propiedades de seguridad: `restricted` falla en cerrado si no hay nada declarado en red, y una clase de inferencia sin declarar se deniega. Corrige código propio de dos días antes; el aviso vino del dueño del proyecto, no de un test | go/internal/modelgateway/gateway.go, go/internal/modelgateway/bundle.go, proto/schemas/model_profile.schema.json |
| TOOL-004 | El worker ejecuta de verdad, a través del Tool Gateway | `DONE` | `test_worker_tool_call_executes_through_the_gateway_and_deduplicates` en verde (5 tests, `make test-python-integration`) contra un `aeon-toolgw` **real** con la política Cedar real y la tabla de `TOOL-005` en Postgres real — un doble habría verificado la forma de la petición y nada sobre política ni dedupe, que es todo el objetivo. Cubre: una tool permitida **se ejecuta de verdad** (el resultado viene del executor Go, no del fichero que solo anotaba intenciones), un repetido del mismo paso **no la vuelve a ejecutar**, `shell.exec` se deniega en el camino real con error **no reintentable** (una tool fuera del manifiesto no pasa a estar permitida por insistir), una llamada **sin principal** se rechaza en vez de ejecutarse sin atribuir, y una configuración contradictoria se rechaza en vez de elegir en silencio. **Dos hallazgos de por medio, ambos invisibles hasta ahora:** (1) el worker **no tenía identidad de agente** — `agent_manifest_ref` llegaba a la API, lo usaba el circuit breaker de A5, y nunca se pasaba al workflow, así que la política por agente no se podía aplicar desde ahí ni en principio; (2) el compose **ya tenía `AEON_TOOLGW_ADDR` puesto en el worker** y el código nunca lo leía — cableado declarado que no hacía nada. Arregla de una vez los **cinco** adaptadores de frameworks (Modo B), que pasan todos por esta función. El modo de ejecución es **explícito y sin default**: un worker sin configurar falla ruidosamente en vez de usar el fichero, porque adivinar mal se parece exactamente a funcionar | python/aeon_worker/activities/tool_activities.py, python/aeon_worker/graph.py, go/internal/runcontroller/controller.go |
| TOOL-005 | Tabla de deduplicación de ejecución en Postgres | `DONE` | `TestToolExecutionIsDeduplicatedByIdempotencyKey` en verde (7 subtests) contra Postgres real y la política Cedar real. El instrumento del test es un **contador de ejecuciones de la tool**, no la respuesta: afirmar sobre la respuesta pasaría igual contra un gateway que ejecutó dos veces y devolvió la segunda. Cubre: 4 llamadas con la misma clave → la tool corre **una vez**; 8 concurrentes → **una vez**; una segunda llamada con la primera **genuinamente en vuelo** → `409` con `Retry-After` y la tool no entra; misma clave con **argumentos distintos** → `409`, porque la clave se deriva de los argumentos y deduplicar ahí daría el resultado de una llamada a otra; un fallo **libera** la clave y cuenta el intento; y la política corre **antes** de la dedupe, así que una tool denegada ni se ejecuta ni deja reclamación. **Tres estados explícitos** (`in_flight`/`completed`/`released`) y no derivados de qué columnas son nulas — la primera versión los dedujo de `completed_at` y una reclamación liberada quedaba indistinguible de una en vuelo, lo que habría bloqueado el reintento para siempre. **La reclamación se escribe ANTES de ejecutar**: registrarla después deja una ventana donde una caída pierde el registro y el reintento repite el efecto, que es justo lo que la tabla existe para impedir | go/internal/store/tool_executions.go, go/internal/api/tool_gateway_handlers.go, go/internal/store/schema.sql |
| TOOL-006 | `search.rag` real sobre documentos | `DONE` | `TestSearchRagReturnsRealPassagesFromIndexedDocuments` en verde (5 subtests) contra Postgres+pgvector real **y embeddings reales de Prometheus** (`qwen3-embedding`, dimensión 1024 medida contra el despliegue el 2026-09-14). La consulta del test **no comparte vocabulario** con el pasaje que la responde —«¿qué ocurre si un paso se ejecuta dos veces?» contra un texto sobre claves de idempotencia—, así que una búsqueda por palabras clave fallaría: es la única forma de que el verde signifique recuperación semántica y no `grep`. El locator se verifica **contra el documento original**: el rango de bytes tiene que contener de verdad el texto que etiqueta, porque un locator que apunta a otro sitio es peor que ninguno — el Citation Verifier lo daría por bueno. **La propiedad de seguridad que más importa:** `embedding_model` se guarda **por trozo**, y buscar con un modelo distinto del que indexó se **rechaza**. Ese fallo no tiene síntoma — dos espacios vectoriales distintos se comparan perfectamente bien y devuelven pasajes seguros, plausibles y equivocados, y nada aguas abajo puede notarlo. Más: reindexar converge en vez de duplicar, y un corpus vacío se distingue de «sin buenas coincidencias». El `Embedder` es **una interfaz que no nombra proveedor** (regla de `MDL-017`); la implementación vive con el adaptador que sabe hablar con la plataforma. Entrada de datos por `aeon rag index <dir> [--corpus]` | go/internal/rag/, go/internal/store/rag.go, go/internal/toolexec/rag_tool.go, go/cmd/aeon/rag.go |
| TOOL-007 | `search.web` real | `DONE` | `TestSearchWebReturnsRealResults` en verde (8 subtests), el primero contra **internet real** vía una instancia de SearXNG en `deploy/compose`. Antes [`executor.go`](go/internal/toolexec/executor.go) registraba `search.web` devolviendo sus propios argumentos, así que un run la llamaba, leía `{"status": "executed"}` y no recuperaba nada — y los cinco adaptadores de framework pasan por ese executor, así que **ninguno buscaba**. Ahora se registra **solo si hay proveedor configurado** (patrón de `TOOL-006`): sin él, `unknown tool` en la primera llamada en vez de un éxito vacío. **Verificado de punta a punta por el camino gobernado**, no solo en proceso: `POST /execute` con el principal real y las 4 políticas Cedar del bundle devuelve URLs de internet (`allowed:true`), `shell.exec` sigue dando `403 denied by policy`, y `search.rag` —permitida pero sin credenciales en este despliegue— da `404 unknown tool`. Las tres respuestas son distintas; antes la primera y la tercera eran indistinguibles. **Proveedor no acoplado:** la interfaz `websearch.Searcher` no nombra ninguno, y `InNetwork()` es **declarado y no inferido** (igual que `modelgateway.Candidate.InNetwork`) porque la consulta la escribe el modelo desde el contexto del run y puede llevar fragmentos de lo que esté leyendo: mandarla a un tercero es una decisión de egress. SearXNG es self-hosted, así que no sale de la red. **Tres hallazgos de medir la instancia real antes de escribir:** (a) `number_of_results` llega `null`, así que no se parsea —un adaptador que lo creyera leería «cero resultados» de un campo sin rellenar—; (b) `unresponsive_engines` llega como `[["duckduckgo","CAPTCHA"]]` de forma continua mientras Google contesta, lo que obliga a **tres estados**: resultados con algún motor caído es normal, cero resultados con **todos** caídos es **error nombrando cuál y por qué** (devolverlo vacío le diría al agente que la web no tiene nada y razonaría desde ahí), y cero resultados sin motores caídos es una respuesta válida; (c) el JSON **no está activo por defecto** en SearXNG y contesta `403` justo a la petición de la que dependemos, así que el `healthcheck` del compose comprueba el JSON y no el puerto — uno de puerto pasaría | — |
| OBS-007 | Reconciliar nuestra contabilidad con la de la plataforma | `DONE` | `TestLedgerTotalsReconcileWithPlatformUsage` en verde (4 subtests) **contra la plataforma real**. Faltaban dos cosas y no existía ninguna: **(1) la clave para unir las dos tablas.** El id de la plataforma llega en la **cabecera** `x-request-id`; el cuerpo trae `id: chatcmpl-...` y las cabeceras traen además `x-trace-id`, y `GET /v1/usage/{id}` da **404 a esos dos** — medido. Ahora se captura, viaja en la respuesta normalizada y se guarda en `provider_request_id` (nullable: «este proveedor no da id» y «no lo registramos aún» son hechos distintos y solo uno es reconciliable). **(2) una cifra suya legible**: está, sin `admin:read`, y **con las tarifas aplicadas**, que es lo que convierte esto en comparar dos derivaciones en vez de copiar un número — recalcular detecta un fallo suyo sin preguntarles (`platform cost 1.14e-05`, `recomputed 1.14e-05`). Cinco tipos de divergencia, y **una fila sin clave se cuenta `Unreconciled`, nunca como acuerdo**: contar lo no comparado como coincidencia es la forma del `skip` que cuenta como verde. **Y encontró una divergencia real en su primera ejecución, que es para lo que existe:** el bundle declaraba los modelos locales `compute_based` sin tarifas —por la suposición del plan de que la inferencia local se factura por GPU-segundo— y **este despliegue factura por token** (`qwen3-0.6b` 0.2/0.6, `gpt-oss-20b-mxfp4` 0.3067/1.052 por 1M, leídas de la plataforma). Corregido a `token_based` con esas tarifas. Eso reencuadra `OBS-008`/`OBS-009`: siguen siendo correctas —el estado «sin tarifar» existe, hay once filas así en su plataforma— pero el caso que las motivó era **un error nuestro de declaración**. **Lo que el reconciliador NO afirma, escrito en su doc:** el acuerdo no dice que el gasto sea correcto, porque sus tarifas son **precios** alineados con modelos abiertos alojados y no lo que les cuesta la GPU; cuadrar al céntimo dice que nuestra derivación coincide con la suya. Hacer `cost_model` obligatorio destapó además que **`aeon init` generaba un manifiesto sin él**, así que un proyecto recién creado producía modelos sin tarifar en silencio — corregido. `TestLedgerTotalsReconcileWithPlatformUsage` en verde — hoy Prometheus y Aeon mantienen **dos contabilidades independientes de las mismas llamadas** y no hay nada que las compare. Nuestro ledger (`OBS-003`) escribe su propia fila por llamada del gateway con la tarifa del `ModelPolicyBundle`; su export de uso escribe la suya por petición. Miden cosas distintas a propósito —plataforma por petición, nosotros por perfil y por run, que es lo que mira quien tiene presupuesto— pero **si discrepan, nadie se entera**: no hay ningún punto del sistema donde las dos cifras se toquen. No se trata de unificarlas: se trata de que exista **una** comprobación que las compare, porque una diferencia sistemática entre ambas es justo el error que ninguna puede detectar sola. Descubierto al contestar a Axonium, que daba por hecho que leíamos su export — **no lo leemos en ningún sitio**, cero referencias a `/v1/usage` en el repo, y esa buena noticia es la que destapa esta. **Respuesta de Prometheus vía Axonium (2026-09-22), y cambia qué es esta feature en dos sentidos.** (a) **No hay que copiar su cifra: se puede recalcular.** Las tarifas viajan en cada fila del export, y `(prompt × precio_prompt + completion × precio_completion) / 1e6` cuadra en **1946 de 1946** filas con tarifa. Así que esto sí es comparar dos derivaciones independientes y no copiar un número — si una no cuadra, es fallo suyo y se ve sin preguntar. (b) **Y es un PRECIO, no un COSTE**: sus tarifas base están alineadas con precios publicados de modelos abiertos alojados, no con lo que les cuesta la GPU; el camino derivado del coste real existe pero es una acción manual del operador por modelo. Si reconciliamos gasto de infraestructura contra eso, **cuadrar al céntimo no diría que el gasto es correcto, diría que los dos lados copiaron la misma lista**. Decidir qué medimos es parte de esta feature y no un detalle de implementación. También confirmado: `cost_usd` se guarda al escribir y no se recalcula al leer —lo cambiaron porque antes un cambio de precio re-tarificaba el histórico entero en la siguiente consulta—, y hoy **0 de 1951 filas tienen `cost_usd = null`**, pero el nulo sigue siendo alcanzable por tres vías (media tarifa, modelo archivado nunca sembrado, modelo de imagen sin precio por imagen), lo que refuerza la columna anulable de `OBS-008`. Y no hay que negociar el formato: `model_id` y `model_slug` viajan los dos desde `PRM-113`, así que conectarlo no lleva migración | — |
| OBS-008 | El ledger de coste deja de escribir ceros que mienten | `DONE` | `TestCostLedgerDistinguishesUnpricedFromFree` en verde (5 subtests) contra Postgres real y el handler real. **Verificado con dos mutaciones, no solo con el verde:** devolver el `coalesce(sum(cost_usd), 0)` mata tres subtests —esa sola línea bastaba para resucitar el defecto en el panel aunque la fila estuviera bien, así que arreglar solo la columna no habría servido— y devolver el `return` temprano de `recordCost` mata el de la fila ausente. La segunda mutación destapó además un defecto **del propio test**: usaba un nombre de modelo fijo y acumulaba filas entre ejecuciones, así que pasaba contra una base limpia y habría fallado la segunda vez; ahora cada subtest lleva su modelo y su tabla de tarifas, y se comprueba corriéndolo dos veces seguidas contra la misma base. El subtest de la tarifa de $0 real es el que impide cambiar un hecho fusionado por otro: un cero medido tiene que seguir guardándose como cero. `TestCostLedgerDistinguishesUnpricedFromFree` en verde. Dos defectos medidos el 2026-09-19 al contestar el aviso de Axonium sobre el renombrado de modelos, y ninguno falla nunca: **(a)** toda llamada a un proveedor `compute_based` —es decir, todas las de `prometheus_inference`, el único que usamos de verdad— se escribe con `cost_usd = 0` en una columna `NOT NULL DEFAULT 0`, porque la tarifa solo sabe multiplicar tokens; «nadie lo calculó» y «costó cero» quedan indistinguibles y `TotalsByModel` suma y enseña $0.00. **(b)** si un modelo se renombra, `Pricing.Rate` falla el `map[provider/model]` y [`recordCost`](go/internal/api/model_gateway_handlers.go:91) sale por el `return` temprano **antes** de escribir en el ledger: la llamada no queda con precio nulo, desaparece de la contabilidad entera. Es la misma enfermedad de tres estados que ya arreglamos en `Usage` (`MDL-012`, `MDL-016`), sin aplicar a la columna de coste. Agrupar por `model` (un TEXT público y renombrable) es además el fallo exacto que Prometheus acaba de corregir en sus propias filas partiendo un histórico en dos. **Ampliación de Axonium (2026-09-20), que cambia el arreglo:** la cifra `compute_based` no hay que calcularla, hay que **leerla** — `GET /v1/usage/{request_id}` devuelve `cost_usd` para los cuatro tipos invocables (chat `4.8e-06`, embeddings `1.311e-06`, rerank `2.96e-06`, vision `8.25275e-05`), sin `admin:read`. Así que lo que falta no es aritmética: es que **la columna pueda no tener valor**. Mientras siga `NOT NULL DEFAULT 0`, un `cost_usd` nulo —tarifa sin configurar, modelo nuevo— vuelve a convertirse en «costó cero» y reaparece el mismo `$0.00` por otra puerta, con otro origen y el mismo panel. Los tres SDK de Axonium lo tipan `float | None` justo por esto. **No depende de `OBS-007` ni de que Prometheus conteste `A-22`** (cuándo es `null`): la columna anulable se hace igual, y «no lo he visto» no es «no ocurre» | — |
| OBS-009 | Un coste sin tarifar no puede contar como $0 en el circuit breaker | `DONE` | `TestUnpricedRunsDoNotSilenceTheCostBreaker` en verde (4 subtests). `Observation.CostUSD` y `recordOutcomeRequest.CostUSD` son punteros, y el promedio se calcula **solo sobre las runs tarifadas — en la suma y en el divisor**; meter lo desconocido en el denominador es el mismo defecto con el cero movido de sitio. El veredicto dice «over the last N **priced** run(s)» y **declara cuántas no cubre**, por lo mismo que el panel de `OBS-008`. **Mutación:** devolver lo no tarifado al divisor mata el subtest con su mensaje. Los otros tres subtests son los que impiden cambiar un defecto por otro: un **cero medido** sigue contando y sigue diluyendo (si no, el breaker saltaría con gasto que no es anómalo); una ventana **sin ninguna** run tarifada **no salta** (poner en cuarentena por un gasto que nadie conoce es el fallo espejo, y el que introduce un arreglo descuidado); y el chequeo de tasa de fallo es **independiente** de que el coste se desconozca. **Y cierra la causa un paso antes:** `cost_model` pasa a ser **obligatorio** en `model_profile.schema.json` — `PricingRates` se salta un candidato sin él, así que un modelo quedaba sin tarifar **por omisión**. Declarar `compute_based` sin precio por token sigue siendo válido; lo que deja de serlo es llegar ahí porque nadie escribió el campo. Es la regla de Prometheus, adoptada: añadieron dos modalidades al registro y no a la tabla de precios, y once filas de uso acabaron con coste `null` — «no tener precio tiene que ser una decisión que alguien tomó, no una línea que nadie escribió». Los 5 candidatos del bundle de ejemplo ya lo declaraban. `TestUnpricedRunsDoNotSilenceTheCostBreaker` en verde. `A5` promedia coste por run y `recordOutcomeRequest.CostUSD` es `float64` con `omitempty`: un run sin tarifar llega como **0** y **baja la media**, así que cuanto más gasto no tarifado hay, más seguro parece todo. Es fail-open en un control de seguridad. **Medido el 2026-09-23**, con el mismo gasto real de $20 en los dos casos: diez runs de $2.00 dan `tripped=true` («average cost $2.0000 over the last 5 run(s) exceeds threshold $1.0000») y **los mismos $20** intercalados con diez runs sin tarifar dan `tripped=false`. Y muerde justo donde vivimos: `prometheus_inference` es `compute_based`, así que **hoy toda llamada local es no tarifada** y contribuiría 0. **Latente, no activo**: `DefaultThresholds.MaxAvgCostUSD` es 0, que desactiva el chequeo de coste entero — el defecto se dispara el día que alguien lo active, que es cuando menos se mira. Mismo hallazgo que `OBS-008` una capa más arriba, y lo destapó Axonium contando que Prometheus tenía `sum(row["cost_usd"] or 0.0)` en su **página de facturación**: filas con guion, total `USD 0.00`. Su arreglo sirve de plantilla y cubre el caso no obvio: subtotal nulo cuando nada se tarifó, impuesto y cambio de moneda **propagan** el nulo en vez de multiplicar un desconocido, y una cobertura parcial viene acompañada de cuántas peticiones no cubre | — |
| OBS-006 | Un replay idempotente no se suma dos veces al coste | `TODO` | `TestIdempotentReplayIsNotBilledTwice` en verde — Axonium publicó `meta.idempotent_replay`: el `usage` de un replay **describe la generación original**, así que un reintento con clave de idempotencia devuelve consumo ya contabilizado y nuestro `recordCost` lo sumaría otra vez. Misma familia que `MDL-014` (el cero fabricado) y que `MDL-012` (la caché invisible): no falla, cuadra mal — y aquí **al alza**, que es la dirección que nadie audita porque parece prudente. El arreglo es leer el campo y registrar el evento sin volver a acumular su consumo, conservando la traza de que hubo replay | — |
| MDL-016 | El razonamiento no se pierde ni se paga a ciegas | `DONE` | `TestReasoningContentSurvivesNormalization` en verde (4 subtests). El cuerpo del test es la **transcripción de una respuesta real** del despliegue (2026-09-13, `gpt-oss-20b-mxfp4`): `content` vacío, todo el presupuesto en `reasoning_content`, `finish_reason: length` y 20 tokens facturados — no es un caso raro, es lo que hace cualquier modelo de razonamiento con presupuesto ajustado. **Los dos adaptadores se ejercitan contra el mismo cuerpo**, porque parsean por separado y nada los obligaba a coincidir: una discrepancia significaría que la misma respuesta se normaliza de dos formas según quién la sirvió. Cubre además que el razonamiento **no se concatena** al contenido (mezclarlos no se puede deshacer aguas abajo), que en streaming llega **en su propio flujo y antes** que la respuesta (quien solo mire `content` no ve nada durante toda la fase de pensar, indistinguible de un stream colgado), y que un proveedor que no lo desglosa **no dice cero** sino nada. `reasoning_tokens` llega hasta el ledger de FinOps, que es la mitad de «ni se paga a ciegas» | go/internal/providers/provider.go, go/internal/providers/streaming.go, go/internal/store/finops_ledger.go |
| MDL-015 | El pipeline habla con Prometheus de verdad | `DONE` | `test_deep_research_against_real_prometheus` en verde: `claims=3 report_chars=1188`. Pipeline completo (Planner → Researchers → Sufficiency Gate → Reporter → Citation Verifier) contra Temporal real, worker real en su proceso, `aeon-modelgw` real, **inferencia real en Prometheus** y `search.web` recuperando URLs de internet. Nada falseado. Cierra el hueco 3: `DX-001` probaba la orquestación contra un doble que devolvía JSON bien formado por construcción y `MDL-006` el adaptador con un modelo; nada preguntaba si un modelo real sobrevive un pipeline que valida cada paso contra un esquema. **Siete defectos encontrados, ninguno en el pipeline:** **(1)** el scope OAuth era una cadena escrita a mano con **un** modelo mientras el enrutado elige por petición, así que todo otro candidato daba `403` — **la cadena de fallback del bundle era inusable en producción**; ahora se deriva del bundle (medido: un token admite varios `model:` a la vez). **(2)** `ResearchSubtaskInput` **no tenía** `agent_manifest_ref`, y `TOOL-004` rechaza una llamada sin principal, así que este perfil solo podía usar el ledger que no ejecuta nada. **(3)** el prompt del Researcher **no nombraba las herramientas**: el modelo contestaba de memoria con `FINISH`, produciendo **un informe coherente sin una sola fuente** — lo que `DR-005` existe para impedir. Invisible con el doble, que emitía el nombre correcto. La lista viene ahora de `tools.allow`, no escrita en el prompt: nombrar una que la política deniega invita a una llamada que será rechazada. **(4)** `parse_decision` decía «not valid JSON: ''» ante contenido vacío, nombrando el problema equivocado; ahora informa `finish_reason` y los caracteres de razonamiento, y **el consejo depende de `finish_reason`** — la primera versión decía «sube max_tokens» siempre y el caso real vino con `stop` y 85 caracteres, o sea que el presupuesto no era el problema. **(5)** ningún perfil fijaba `max_tokens`: dependían de un valor que nadie en el repo escribió. **(6)** un `200` vacío no hacía fallar al candidato, así que el gateway devolvía la respuesta vacía de un modelo de razonamiento y **el segundo candidato —el que funciona— nunca se probaba**; medido: `gpt-oss-20b-mxfp4` responde el prompt del Researcher con `content: ""` y `finish_reason: stop`, `qwen3-0.6b` con JSON válido 4/4. `finish_reason: length` se excluye a propósito (es informativo) y una respuesta solo con tool calls también (es usable). **(7)** el modelo envuelve el JSON en vallas de markdown y luego le añade un punto final, **con el prompt diciendo lo contrario** — dos acomodaciones seguidas cuya lección no es sobre el modelo: **una instrucción es una petición**, el gateway descarta `response_format` y la única restricción real no llega. `extract_json_object` localiza el primer objeto completo con `raw_decode` y **deja la frontera escrita**: el objeto sigue teniendo que ser válido y seguir validando contra el esquema, solo se tolera lo que lo rodea; un objeto truncado sigue fallando. Aplicado en los tres parseos (`decision`, `planner`, `reporter`), no solo en el que reventó. **Hallazgo de plataforma, al canal:** `500` «output does not match the expected peg-native format» en **2 de 12** con el prompt del Researcher y **0 de 20** con el del Planner — correlaciona con el prompt, no es aleatorio. `sufficient=False` con 3 claims es correcto: el plan pidió 4 subtareas y una no aportó evidencia `TestDeepResearchAgainstRealPrometheus` en verde — el ejemplo completo contra la plataforma real, no un doble HTTP, con el `ModelPolicyBundle` apuntando a modelos que existen. Cierra el hueco 3: hoy `DX-001` prueba el pipeline y `MDL-006` prueba el adaptador, **y nada prueba los dos juntos** | — |
